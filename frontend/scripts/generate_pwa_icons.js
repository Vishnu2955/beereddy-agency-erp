...
// Helper to construct a valid uncompressed/deflated RGBA PNG buffer
function createBrandedPng(width, height) {
  // RGBA raw pixel data (scanline width * 4 + 1 filter byte per row)
  const rowSize = width * 4 + 1;
  const rawData = Buffer.alloc(rowSize * height);

  // Colors
  // Navy #0F172A (15, 23, 42)
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      ...
      // Draw white V logo shape in center
  ...
  // Compress raw pixel buffer with zlib DEFLATE
...
sizes.forEach((s) => {
  ...
  // Write top-level icon-192.png and icon-512.png for legacy fallback
